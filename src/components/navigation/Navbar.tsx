...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // If no exact match (e.g. between paddings), find the closest section above triggerPoint
      if (!detectedSection) {
    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-[9980] transition-all duration-500 ${
          isScrolled
            ? 'py-3 bg-[#050505]/90 backdrop-blur-2xl border-b border-white/[0.08] shadow-[0_10px_30px_rgba(0,0,0,0.85)]'
            : 'py-6 bg-transparent'
        }`}
      >
        <div className="max-w-[1540px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <Magnetic strength={0.2}>
            ...
            >
              ...
              />
              <span className="font-display text-sm md:text-base font-extrabold tracking-widest text-foreground uppercase">
                {studioConfig.name}
              </span>
            </a>
          </Magnetic>

          {/* Desktop Navigation Links with Precise Active Scroll Spy */}
          <nav className="hidden md:flex items-center gap-1.5 lg:gap-2 p-1.5 rounded-full bg-white/[0.03] border border-white/[0.06] backdrop-blur-md">
              ...
              return (
                <Magnetic key={link.label} strength={0.15}>
                  <a
                    ...
                    className={`relative px-4 py-1.5 rounded-full font-mono text-[11px] uppercase tracking-widest transition-all duration-300 flex items-center gap-1.5 select-none ${
                      isActive
          ...
          {/* Far-Right: Tape Sound Toggle, Live Clock & Quick Action */}
          <div className="hidden md:flex items-center gap-4 lg:gap-5">
            <Magnetic strength={0.25}>
              <SoundToggle />
            </Magnetic>
            <div className="w-[1px] h-3.5 bg-white/10 hidden lg:block" />
            ...
            <Magnetic strength={0.3}>
              <a
                ...
                className={`px-4 py-2 rounded-full border transition-all duration-300 text-[11px] font-mono uppercase tracking-wider ${
          ...
          {/* Mobile Right Controls: Compact Sound Toggle & Menu Trigger */}
          <div className="flex md:hidden items-center gap-3">
            <SoundToggle compact />
            <button
              onClick={() => setMobileOpen(!mobileOpen)}
              className="flex flex-col items-end justify-center w-8 h-8 gap-1.5 focus:outline-none"
              aria-label="Toggle Navigation Menu"
              aria-expanded={mobileOpen}
            >
              <span
                className={`block h-[2px] bg-foreground transition-all duration-300 ${
                  mobileOpen ? 'w-6 rotate-45 translate-y-2 bg-dx-purple-bright' : 'w-6'
                }`}
              />
              <span
                className={`block h-[2px] bg-foreground transition-all duration-300 ${
              ...
              <span
                className={`block h-[2px] bg-foreground transition-all duration-300 ${
                  mobileOpen ? 'w-6 -rotate-45 -translate-y-2 bg-dx-purple-bright' : 'w-5'
                }`}
              />
            </button>
          </div>
