...
interface ProjectCardStickyProps {
  project: Project;
  onSelect: (project: Project) => void;
}

export function ProjectCardSticky({ project, onSelect }: ProjectCardStickyProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b border-white/[0.08] pb-5 sm:pb-6 mb-6 sm:mb-8">
        <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
          <span className="font-mono text-sm md:text-base font-bold text-dx-purple-bright">
            {project.number}
          </span>
          <span className="text-white/20">/</span>
          <span className="font-mono text-[11px] sm:text-xs md:text-sm uppercase tracking-widest text-foreground-secondary">
            {project.category}
          </span>
        </div>
        <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4">
          <span className="font-mono text-[10px] sm:text-xs uppercase tracking-wider text-foreground-secondary/60">
            {project.role}
          </span>
          {project.liveUrl && (
            <button
              onClick={(e) => handleOpenLive(e, project.liveUrl!)}
              className="px-3.5 py-1.5 rounded-full bg-dx-purple/20 hover:bg-dx-purple border border-dx-purple-bright/40 text-[11px] sm:text-xs font-mono text-white font-bold transition-all duration-300 flex items-center gap-1.5 cursor-pointer whitespace-nowrap"
            >
      ...
      {/* Main Grid: Typography & Massive Visual */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-12 items-center">
        <div className="lg:col-span-6 space-y-4 sm:space-y-5">
          <h3 className="font-display text-[clamp(1.85rem,7vw,3.25rem)] font-black uppercase tracking-tight text-white group-hover:text-dx-purple-bright transition-colors duration-500 leading-tight break-words max-w-full">
            {project.title}
          </h3>
